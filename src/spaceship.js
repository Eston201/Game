...
class player {
  constructor(params) {
    ...
    this.keyboard = new THREEx.KeyboardState(); // new keyboardstate to listen to keyboard presses for rocket controls
...
*/
  ...
  Update(delta){
    ...
    var moveDistance = 300 * delta; // travel at 400 pixels per second
    var rotateAngle = Math.PI / 220
    //make rings animate
    this.ring1.rotation.y += 0.03;
    ...
    this.prod.translateZ(-moveDistance);
    this.params.camera.translateZ(-moveDistance*0.98); //0.99 : move slower than rocket to create offset
    //this.params.camera.translateY(1); //0.99 : move slower than rocket to create offset
    ...
    if (this.keyboard.pressed("W")) {
      //increase the forward speed of the aircraft look above for befault speed
       this.prod.translateZ(-(moveDistance+acceleration));
       this.params.camera.translateZ(-(moveDistance*0.98 + acceleration));
    ...
    if (this.keyboard.pressed("A")) {
     this.prod.rotateOnAxis( new THREE.Vector3(0,0,1), rotateAngle);
    }
  ...
  if ( this.keyboard.pressed("up") ){
    this.prod.rotateOnAxis( new THREE.Vector3(1,0,0), -rotateAngle);
  }
  ...
  if ( this.keyboard.pressed("right") ) {
    this.prod.rotateOnAxis( new THREE.Vector3(0,0,1), -rotateAngle-0.002);
    this.prod.rotateOnAxis( new THREE.Vector3(0,1,0), -rotateAngle);
  }
if ( this.keyboard.pressed("left") ) {
    ...
    //if we are not pressing a or d then tilt the plane slowly back to its position
   ...
   // this.params.camera.translateZ(-moveDistance*0.98);
    //this.tcamera.Update(delta);//update the camera to follow plane
    //this.UpdateCamera(delta,this.params.camera);
    this.params.camera.lookAt( this.prod.position);  //look at rocket
    //this.params.camera.position.set(this.prod.position.x+delta,this.prod.position.y+delta,this.prod.position.z+delta);
  }
